...
import org.hexils.dnarch.da.objects.Trigger;
import org.hexils.dnarch.da.objects.actions.DestroyBlock;
import org.hexils.dnarch.da.objects.actions.ReplaceBlock;
import org.hexils.dnarch.da.objects.actions.Spawn;
import org.hexils.dnarch.da.objects.actions.Type;
import org.hexils.dnarch.da.objects.conditions.Distance;
                    ...
                    if (dm.isEditing()) {
                        if (args.length > 2) {
                            switch (args[1]) {
                                ...
                                case "condition" -> {
                                    for (org.hexils.dnarch.da.objects.conditions.Type t : org.hexils.dnarch.da.objects.conditions.Type.values())
                                        s.add(t.name().toLowerCase());
                                }
                            }
                        } else {
