...
public final class dc_cmd implements CommandExecutor {
            ...
            case "dungeon" -> {
                if (args.length > 1) {
                    switch (args[1].toLowerCase()) {
                        ...
                        case "start" -> dm.getCurrentDungeon().start();
                        case "edit" -> {
                            Dungeon d;
                            if (args.length == 2) {
    ...
    public static final List<Map<String, List<String>>> defTAb;
    static {
        defTAb = List.of(
                Map.of(
                        "create", List.of("dungeon")
                )
        );
    }
    public static final List<Map<String, List<String>>> editTab;
    static {
        editTab = List.of(
                Map.of(
                        "create", List.of("action", "condition", "trigger", "section")
                ),
                Map.of(
                        "action", Arrays.stream(org.hexils.dnarch.objects.actions.Type.values()).map(Enum::name).toList(),
                        "condition", Arrays.stream(org.hexils.dnarch.objects.conditions.Type.values()).map(Enum::name).toList()
                )
        );
    }
    public static List<String> getTab(int l, String arg, boolean edit) {
        List<String> s = defTAb.get(l).getOrDefault(arg, new ArrayList<>());
        if (edit) s.addAll(editTab.get(l).getOrDefault(arg, new ArrayList<>()));
        return s;
    }
    ...
    public static @NotNull List<String> complete(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String @NotNull [] args) {
        ...
        if (args.length == 0) {
            ...
            default -> {
                if (args.length >= 2) return getTab(args.length-1, args[args.length-1], dm.isEditing());
            }
        }
        return s;
    }
